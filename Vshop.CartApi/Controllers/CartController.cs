...
public class CartController : ControllerBase
{
    ...
    [HttpPost("checkout")]
    public async Task<ActionResult<CartDTO>> Checkout(CheckoutHeaderDTO checkoutHeaderDTO)
    {
        var cart = await _repository.GetCartByUserIdAsync(checkoutHeaderDTO.UserId);
            

        if (cart is null)
        {
            return NotFound($"CartHeader Not found for userId = {checkoutHeaderDTO.UserId}");
        }
